...
class DownLoadHandle {
    ...
    private ExecutorService mGetFileService = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors() + 1);
        ...
        while (iterator.hasNext()) {
            ...
            if (dataList.size() > 0) {
                ...
                if (!TextUtils.isEmpty(dataList.get(0).lastModify)) {
                    mResponseCall = HttpHelper.create(DownLoadApi.class)
                        .getHttpHeaderWithIfRange(downLoadEntity.url, dataList.get(0).lastModify, "bytes=" + 0 + "-" + 0);
                }
            } else {
                mResponseCall = HttpHelper.create(DownLoadApi.class).getHttpHeader(downLoadEntity.url, "bytes=" + 0 + "-" + 0);
            }
