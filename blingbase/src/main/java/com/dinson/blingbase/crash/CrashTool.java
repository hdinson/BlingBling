...
public class CrashTool {
    ...
    private static CrashProfile config = new CrashProfile();
        ...
        try {
            if (context == null) {
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(TAM_HANDLER_PACKAGE_NAME)) {
                    Log.e(TAG, "TCrashTool was already installed, doing nothing!");
                } else {
                    if (oldHandler != null && !oldHandler.getClass().getName().startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
                        Log.e(TAG, "IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this is correct? If you use a custom UncaughtExceptionHandler, you must initialize it AFTER TCrashTool! Installing anyway, but your original handler will not be called.");
                    }
                    ...
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        ...
                        public void uncaughtException(@NonNull Thread thread, @NonNull Throwable throwable) {
                            if (config.isEnabled()) {
                                Log.e(TAG, "App has crashed, executing TCrashTool's UncaughtExceptionHandler", throwable);
                ...
                Log.i(TAG, "TCrashTool has been installed.");
            }
        } catch (Throwable t) {
            Log.e(TAG, "An unknown error occurred while installing TCrashTool, it may not have been properly initialized. Please report this as a bug if needed.", t);
        }
    }
    ...
    public static String getStackTraceFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(CrashTool.EXTRA_STACK_TRACE);
    }
     ...
     */
    @Nullable
    public static CrashProfile getConfigFromIntent(@NonNull Intent intent) {
        CrashProfile config = (CrashProfile) intent.getSerializableExtra(CrashTool.EXTRA_CONFIG);
    ...
    public static String getActivityLogFromIntent(@NonNull Intent intent) {
        return intent.getStringExtra(CrashTool.EXTRA_ACTIVITY_LOG);
    }
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull CrashProfile config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull CrashProfile config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull CrashProfile config) {
        if (config.getEventListener() != null) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static CrashProfile getConfig() {
        return config;
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull CrashProfile config) {
