package dinson.customview.widget._013stepview;
...
import dinson.customview.widget._013stepview.bean.StepBean;
...
public class HorizontalStepsViewIndicator extends View
{
    ...
    private int mUnCompletedLineColor = ContextCompat.getColor(getContext(), R.color._013_uncompleted_color);//定义默认未完成线的颜色  definition
    ...
    private void init()
    {
        ...
        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.drawable._013_complted);//已经完成的icon
        mAttentionIcon = ContextCompat.getDrawable(getContext(), R.drawable._013_attention);//正在进行的icon
        mDefaultIcon = ContextCompat.getDrawable(getContext(), R.drawable._013_default_icon);//未完成的icon
    }
    ...
    protected synchronized void onDraw(Canvas canvas)
    {
        ...
        for(int i = 0; i < mCircleCenterPointPositionList.size() -1; i++)
        {
            ...
            final float preCompletedXPosition = mCircleCenterPointPositionList.get(i);
            //后一个CompletedXPosition
            final float afterCompletedXPosition = mCircleCenterPointPositionList.get(i + 1);
            ...
            if(i <= mComplectingPosition&&mStepBeanList.get(0).getState()!=StepBean.STEP_UNDO)//判断在完成之前的所有点
            {
                ...
                canvas.drawRect(preCompletedXPosition + mCircleRadius - 10, mLeftY, afterCompletedXPosition - mCircleRadius + 10, mRightY, mCompletedPaint);
            } else
            {
        ...
        {
            final float currentCompletedXPosition = mCircleCenterPointPositionList.get(i);
            Rect rect = new Rect((int) (currentCompletedXPosition - mCircleRadius), (int) (mCenterY - mCircleRadius), (int) (currentCompletedXPosition + mCircleRadius), (int) (mCenterY + mCircleRadius));
