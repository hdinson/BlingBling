package dinson.customview.weight._003toast;
...
public class LoadToastView extends AppCompatImageView {
    ...
    private int BASE_TEXT_SIZE = 14;
    private int IMAGE_WIDTH = 32;
    private int TOAST_HEIGHT = 40;
    ...
    public LoadToastView(Context context) {
        ...
        successPaint.setColor(Color.parseColor("#3CCD88"));
        ...
        completeicon = getResources().getDrawable(R.drawable._007toast_navigation_check);
        completeicon.setBounds(iconBounds);
        failedicon = getResources().getDrawable(R.drawable._007toast_error);
    ...
    private void calculateBounds() {
        ...
        int padding = (TOAST_HEIGHT - IMAGE_WIDTH) / 2;
        int textWidth = Math.min(mTextBounds.width(), MAX_TEXT_WIDTH);
        iconBounds = new Rect(TOAST_HEIGHT + textWidth - padding, padding, TOAST_HEIGHT + textWidth - padding + IMAGE_WIDTH, IMAGE_WIDTH + padding);
    ...
    protected void onDraw(Canvas c) {
        ...
        float translateLoad = (1f - ws) * (IMAGE_WIDTH + textWidth);
        ...
        float totalWidth = leftMargin * 2 + th + ws * (IMAGE_WIDTH + textWidth) - translateLoad;

        toastPath.reset();
        toastPath.moveTo(leftMargin + th / 2, 0);
        toastPath.rLineTo(ws * (IMAGE_WIDTH + textWidth), 0);
        ...
        if (outOfBounds) {
            ...
            if (prevUpdate == 0) {
                ...
                if (shift - textWidth > mTextBounds.width()) {
                    prevUpdate = 0;
                }
            }
            c.clipRect(th / 2, 0, th / 2 + textWidth, TOAST_HEIGHT);
            if (!mLtr || getTextDirection() == TEXT_DIRECTION_ANY_RTL) {
                c.drawText(mText, th / 2 - textWidth + shift, yPos, textPaint);
            } else {
                c.drawText(mText, th / 2 - shift + textWidth, yPos, textPaint);
            }
        } else {
            c.drawText(mText, 0, mText.length(), th / 2, yPos, textPaint);
        }
    }
    ...
    private int measureWidth(int measureSpec) {
        ...
        if (specMode == MeasureSpec.EXACTLY) {
            ...
            // Measure the text
            result = IMAGE_WIDTH + Math.min(mTextBounds.width(), MAX_TEXT_WIDTH) + TOAST_HEIGHT;
